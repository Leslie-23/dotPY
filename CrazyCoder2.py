...
def get_weather_data(url):
    ...
    # Find the temperature and time elements
    temp_elements = soup.select('.temperature')  # Replace with actual CSS selector
    time_elements = soup.select('.time')  # Replace with actual CSS selector

    temps = [temp.get_text().strip() for temp in temp_elements]
    times = [time.get_text().strip() for time in time_elements]

    return times, temps

# URL of the weather website
weather_url = 'https://paltech-weather02.netlify.app/'
